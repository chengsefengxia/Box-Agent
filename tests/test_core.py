...
class _ParallelFileTool(Tool):
    ...
    optionally returning the structured receipt required for exact ownership."""
    ...
    def __init__(
        ...
        filename: str,
        *,
        explicit_receipt: bool = False,
    ):
        self._workspace_dir = workspace_dir
        self._filename = filename
        self._explicit_receipt = explicit_receipt
    ...
    async def execute(self, **kwargs):
        ...
        artifact_path = out_dir / self._filename
        artifact_path.write_text("generated by parallel tool")
        # Note: content does NOT reference the filename, so only the diff layer
        # can surface it.
        raw_output = (
            {"type": "artifact", "abs_path": str(artifact_path)}
            if self._explicit_receipt
            else None
        )
        return ToolResult(
            success=True,
...
async def test_parallel_two_tools_artifacts_no_duplicates(tmp_path):
    """Two parallel tools bind their explicit receipts to their own call IDs."""
    (tmp_path / "output").mkdir(exist_ok=True)
    ...
    class _MultiFileTool(_ParallelFileTool):
        ...
        def __init__(self, workspace_dir, filename, tool_name, **kwargs):
            super().__init__(workspace_dir, filename, **kwargs)
            self._tool_name = tool_name

    tool_a = _MultiFileTool(
        str(tmp_path), "a.txt", "make_a", explicit_receipt=True
    )
    tool_b = _MultiFileTool(
        str(tmp_path), "b.txt", "make_b", explicit_receipt=True
    )
    ...
    assert {a.filename: a.tool_call_id for a in artifacts} == {
        "a.txt": "p1",
        "b.txt": "p2",
    }
...
@pytest.mark.asyncio
async def test_parallel_multiple_tools_without_receipts_do_not_guess_owner(tmp_path):
    """A shared batch diff cannot prove which parallel call owns a file."""
    (tmp_path / "output").mkdir(exist_ok=True)
        ...
        LLMResponse(
            ...
            tool_calls=[
                ToolCall(
                    id="p1",
                    type="function",
                    function=FunctionCall(name="make_a", arguments={}),
                ),
                ToolCall(
                    id="p2",
                    type="function",
                    function=FunctionCall(name="make_b", arguments={}),
                ),
            ],
            finish_reason="tool",
        ),
    ...
    events = await collect(
        run_agent_loop(
            ...
            messages=_msgs(),
            tools={
                "make_a": _MultiFileTool(str(tmp_path), "a.txt", "make_a"),
                "make_b": _MultiFileTool(str(tmp_path), "b.txt", "make_b"),
            },
            max_steps=5,
            workspace_dir=str(tmp_path),
            artifact_detection_enabled=True,
...
@pytest.mark.asyncio
async def test_explicit_artifact_receipt_works_when_diff_detection_is_disabled(tmp_path):
    """Shared project roots accept explicit receipts without scanning neighbors."""
    (tmp_path / "output").mkdir(exist_ok=True)
    tool = _ParallelFileTool(
        str(tmp_path),
        "owned.txt",
    ...
    llm = MockLLM([
        LLMResponse(
            content="making a file",
            tool_calls=[
                ToolCall(
                    id="owned-call",
                    type="function",
                    function=FunctionCall(name="make_file", arguments={}),
                ),
            ],
            finish_reason="tool",
        ),
    ...
    artifacts = [e for e in events if isinstance(e, ArtifactEvent)]
    assert [(a.filename, a.tool_call_id) for a in artifacts] == [
        ("owned.txt", "owned-call")
    ]
    tool_result = next(e for e in events if isinstance(e, ToolCallResult))
    assert tool_result.raw_output["session_id"] == "session-a"
    assert tool_result.raw_output["task_id"] == "task-a"
    assert tool_result.raw_output["turn_id"] == "turn-a"
    assert tool_result.raw_output["tool_call_id"] == "owned-call"


@pytest.mark.asyncio
async def test_workflow_declared_artifact_works_when_diff_detection_is_disabled(tmp_path):
    """A trusted primary output stays attributable without a directory scan."""
    output_dir = tmp_path / "output"
    output_dir.mkdir()
    ...
    class _FileTool(Tool):
        ...
        async def execute(self, **kwargs):
            ...
            return ToolResult(success=True, content="created file [qa.json]")

    class _DeclaredArtifactPolicy(_RecoverablePresentationPolicy):
        def declared_artifact_path(self, tool_name, arguments):
            return str(deck_path) if tool_name == "make_file" else None
    ...
    events = await collect(
        run_agent_loop(
            llm=llm,
            messages=_msgs(),
            tools={"make_file": _FileTool()},
            max_steps=5,
            workspace_dir=str(tmp_path),
            artifact_detection_enabled=True,
            artifact_diff_detection_enabled=False,
            artifact_root_dir=str(output_dir),
            session_id="session-a",
            task_id="task-a",
            turn_id="turn-a",
            workflow_policy=_DeclaredArtifactPolicy(),
        )
    )

    artifacts = [event for event in events if isinstance(event, ArtifactEvent)]
    # The trusted declaration is one primary artifact. A filename mentioned in
    # tool output must not expand that receipt into additional ownership.
    assert [(event.filename, event.tool_call_id) for event in artifacts] == [
        ("deck.json", "declared-call")
    ]
    tool_result = next(event for event in events if isinstance(event, ToolCallResult))
    assert tool_result.raw_output["type"] == "artifact"
    assert tool_result.raw_output["path"] == str(deck_path)
    assert tool_result.raw_output["session_id"] == "session-a"
    assert tool_result.raw_output["task_id"] == "task-a"
    assert tool_result.raw_output["turn_id"] == "turn-a"
    assert tool_result.raw_output["tool_call_id"] == "declared-call"
